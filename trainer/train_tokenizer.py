...
# 全局配置
DATA_PATH = "../dataset/sft_t2t_mini.jsonl"  # 用哪个数据文件来训练tokenizer
TOKENIZER_DIR = "../model_learn_tokenizer/"  # 训练好的tokenizer保存到哪里
VOCAB_SIZE = 6400  # 词表大小minimind是6400
SPECIAL_TOKENS_NUM = 36  # 预留多少个特殊token位置
...
def train_tokenizer(
    ...
    # BPE全称Byte Pair Encoding 字节对编码
    tokenizer = Tokenizer(models.BPE())
    # 字节级BPE，先把所有文本都编码成字节序列，然后在字节序列上做
    tokenizer.pre_tokenizer = pre_tokenizers.ByteLevel(add_prefix_space=False)

    special_tokens_list = [
        "<|endoftext|>",  # 常用作pad /unk/文本结束类标记
        "<|im_start|>",  # 一段消息开始
        "<|im_end|>",
        "<|object_ref_start|>",
        "<|object_ref_end|>",
    ...
    trainer = trainers.BpeTrainer(
        ...
        special_tokens=all_special_tokens,  # 强制加入的特殊token,必须再词表里有固定位置，不能被普通BPE切碎
    )
    # 开始训练
    texts = get_texts(data_path)
    # 边读取文本边训练BPE tokenizer
    ...
    # 保存tokenizer.json，这是tokenizers库的原生格式，包含完整的模型、词表、合并规则、添加的token等所有信息
    tokenizer.save(os.path.join(tokenizer_dir, "tokenizer.json"))
    tokenizer.model.save(tokenizer_dir)

    tokenizer_json_path = os.path.join(tokenizer_dir, "tokenizer.json")

    # 读取tokenizer.json，遍历所有added_tokens，把不在 special_tokens_list 里的 token 的 special 设为 False。然后再写会文件

    # 我们希望一些特殊token在比表里作为完整token保留，但不希望它们在decode时被当作需要跳过的特殊token
    ...
    # 构造 tokenizer_config.json，这是 HuggingFace Transformers 库的配置文件，用于 AutoTokenizer.from_pretrained() 加载
    config = {
        "add_bos_token": False,
        "add_eos_token": False,
